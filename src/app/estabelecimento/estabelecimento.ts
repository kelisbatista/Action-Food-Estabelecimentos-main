...
interface Product { id?: string; nome: string; descricao: string; preco: number; quantity: number; isActive: boolean; imageUrl?: string; }
...
export class Estabelecimento implements OnInit {
  ...
  quantity: null as number | null, // <-- novo campo Qtd
  imageUrl: ''
};
  ...
  public filterOrders = (status: OrderStatus | 'todos') => {
  ...
  try {
    if (!this.newProduct.nome || this.newProduct.preco == null || !this.newProduct.quantity || this.newProduct.quantity <= 0) {
      alert('Nome, preço e quantidade (maior que 0) são obrigatórios.');
      return;
    }
    ...
    this.newProduct = { nome: '', descricao: '', preco: null, quantity: null, imageUrl: '' };
  ...
  try {
    ...
    // Atualiza localmente
    let updatedOrder: Orders | undefined;
    ...
    // Se o pedido foi concluído, atualiza estoque
    if (newStatus === 'concluido' && updatedOrder) {
      await this.updateStockAfterOrder(updatedOrder);
    }
  ...
  public async updateStockAfterOrder(order: Orders) {
  try {
    const user = this.ensureUserOrThrow();
    ...
    for (const item of order.items ?? []) {
      ...
      const product = this.products().find(p => p.nome === item.name || p.id === item.id);
      if (!product) continue;

      let newQtd = product.quantity- item.quantity;
      if (newQtd < 0) newQtd = 0;

      const productDocRef = doc(this.db, `${this.getEstablishmentsBasePathForUser(user.uid)}/produtos/${product.id}`);
      await updateDoc(productDocRef, { 
        Qtd: newQtd,
        isActive: newQtd === 0 ? false : product.isActive
      });

      // Atualiza localmente
      this.products.update(products =>
        products.map(p => p.id === product.id ? { ...p, Qtd: newQtd, isActive: newQtd !== 0 ? p.isActive : false } : p)
      );
    }

    console.log('Estoque atualizado após pedido:', order.id);
