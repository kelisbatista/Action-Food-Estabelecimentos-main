...
import { PartialOrders } from '../services/orders.service';

export interface ScheduleDay { open: string; close: string; isClosed: boolean; }
export type WeekSchedule = Record<string, ScheduleDay>;


interface Product { id?: string; nome: string; descricao: string; preco: number; isActive: boolean; imageUrl?: string; quantidade: number; }

type  Status_pedido = 'pendente' | 'concluido' | 'preparando' | 'cancelado';
type PaymentStatus = 'aprovado' | 'recusado';

interface Orders { id?: string; customerName: string; items?: { nome: string; quantidade: number; preco: number }[]; total?: number;
status_pedido?: Status_pedido; paymentStatus?: PaymentStatus; createdAt?: any; descricao?: string; isActive?: boolean; 
price?: number; quantidade?: number; }
...
export class Estabelecimento implements OnInit {
  ...
  public statuses: (Status_pedido | 'todos')[] = [
  'todos',
  'pendente',
  'preparando',
  'concluido',
  ...
  public currentOrderFilter = signal<Status_pedido | 'todos'>('pendente');
  ...
  public pendingOrders = computed(() => this.allOrders().filter(o => o.status_pedido === 'pendente'));
  ...
  public filteredOrders = computed(() => {
    ...
    return orders.filter(o => o.status_pedido === filter);
  });
 ...
 private listenForProducts(): Promise<void> {
  ...
  public async addProduct() {
    try {
      ...
      this.newProduct = { nome: '', descricao: '', preco: null, imageUrl: '', quantidade: 0 };
...
private listenForOrders(): Promise<void> {
  return new Promise((resolve) => {
    try {
      ...
      const estabId = user.uid;

      const ordersCol = collection(this.db, 'orders');
      const qOrders = query(ordersCol, where('estabId', '==', estabId));

      this.unsubOrders = onSnapshot(
        qOrders,
        async (snap: QuerySnapshot<DocumentData>) => { const list: Orders[] = [];
          for (const snapDoc of snap.docs) {
  const data = snapDoc.data() as any;

  let customerName = data.customerName ?? userId;

  // Se tiver userId no pedido, buscamos o nome
  if (data.userId) {
    const userDoc = await getDoc(
      doc(this.db, 'usuarios', data.userId)
    );
...
public async updateOrderStatus(orderId: string | undefined, newStatus_pedido: Status_pedido) {
  if (!orderId) return;
  try {
    const orderDoc = doc(this.db, `orders/${orderId}`);

    await updateDoc(orderDoc, {
      status_pedido: newStatus_pedido
    });

    console.log(`Pedido ${orderId} atualizado para ${newStatus_pedido}`);
  } catch (e) {
    console.error('Erro ao atualizar pedido:', e);
  }
}
  public filterOrders(status_pedido: Status_pedido | 'todos') { this.currentOrderFilter.set(status_pedido); }
  public getOrderCountByStatus(status_pedido: Status_pedido | 'todos'): number {
    if (status_pedido === 'todos') return this.allOrders().length;
    return this.allOrders().filter(o => o.status_pedido === status_pedido).length;
  }
  public getOrderStatusBadgeClass(status_pedido?: Status_pedido): string {
  ...
  public getOrderCardClass(status_pedido?: Status_pedido): string {
    switch (status_pedido) {
